...
// A temple can legitimately report any archived version — including a
// reviewed CFW, and including a version its partner does not share, since a
// split pair is exactly the state an interrupted cross-version update leaves
// behind and exactly when a technician most wants a backup (observed live,
// session UMFU-3L5P: the combined backup refused the split 2.2.6.11/2.2.8.11
// pair whose repair it was gating). Resolution is therefore per route: an
// exact-version archive match, official channel preferred, any verified
// channel accepted. A version with no archived bundle degrades to an explicit
// recorded omission — the case bytes and live temple snapshots are the
// irreplaceable parts of the backup; bundles are re-downloadable.
export function resolveGlassesRecoveryReleases(catalog, templeProbes) {
  ...
  for (const side of ["left", "right"]) {
    ...
    const release =
      ...
      catalog.find((candidate) => candidate.version === version) ??
      null;
    routes[side] = {
      side,
      version,
      ...
      omissionReason: release
        ? null
        : `The archive does not contain a G2 ${version} recovery bundle in any channel.`,
    };
  }
  const releases = [];
  for (const side of ["left", "right"]) {
