...
test("a split-version pair resolves per-route bundles instead of refusing", () => {
  // Observed live (session UMFU-3L5P): the combined backup refused the split
  // 2.2.6.11/2.2.8.11 pair whose repair it was gating. A split pair is the
  // remnant of an interrupted cross-version update — exactly when a backup
  // matters most.
  const resolution = resolveGlassesRecoveryReleases([release, cfwRelease], {
    left: makeProbe("left", "2.2.6.10"),
    right: makeProbe("right", "2.2.8.11"),
  });
  assert.equal(resolution.pairMatched, false);
  assert.equal(resolution.left.release, release);
  assert.equal(resolution.right.release, cfwRelease);
  assert.deepEqual(resolution.releases, [release, cfwRelease]);
});

test("a reviewed-CFW pair matches its archived bundle outside the official channel", () => {
  const resolution = resolveGlassesRecoveryReleases([release, cfwRelease], {
    left: makeProbe("left", "2.2.8.11"),
    right: makeProbe("right", "2.2.8.11"),
  });
  assert.equal(resolution.pairMatched, true);
  assert.equal(resolution.left.release, cfwRelease);
  assert.deepEqual(resolution.releases, [cfwRelease]);
});
