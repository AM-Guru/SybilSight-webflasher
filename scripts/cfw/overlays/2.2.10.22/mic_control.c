 ...
 * 0x200773a5 is nonzero while a phone is connected. Read-only diagnostics. */
#define BLE_CONN_CTX_PTR     ((volatile uint32_t *)0x200765B8U)
#define BLE_CENTRAL_CONNECTED ((volatile uint8_t *)0x200773A5U)
...
__attribute__((used, noinline)) void mic_pcm_tap(uint32_t source, const void *pcm, uint32_t bytes) {
    ...
    if ((ctx->mic_flags & MIC_FLAG_MGR_CTRL) && *CODEC_ON_FLAG) *CODEC_ON_FLAG = 0u;
    ...
    for (uint32_t si = 0; si < slices; si++) {
        uint8_t flags = (uint8_t)(ctx->mic_flags & (MIC_FLAG_BEAMFORM | MIC_FLAG_ARM_HW));
...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if ((ctx->mic_flags & MIC_FLAG_MGR_CTRL) && ctx->mic_source == MIC_SRC_CODEC) {
        ...
        /* 2.2.10.20: no audio-manager slot hold. The table's extent could not be verified and
         * a production-mode arm following a manager-mode session reset the left temple
         * (2026-09-14 10:49). A stock release that powers the codec down under the tap is
         * instead repaired on the next RENEW (see mic_apply_control). */
        ctx->mic_hw_armed = 1;
        FW_CODEC_CTRL(1u);
        return;
    }
...
static void mic_session_stop(customCfwContext *ctx) {
    ...
    if ((ctx->mic_flags & MIC_FLAG_MGR_CTRL) && ctx->mic_source == MIC_SRC_CODEC) {
        FW_PCM_UNREGISTER(MIC_STOCK_OWNER_ID, MIC_PCM_SLOT);   /* restore the stock LC3 fallback */
        /* Only power the codec down when no stock app holds it (LEFT); the RIGHT has no
         * stock users of its DMIC. Read-only look at the app table (ids 1..6). */
        uint8_t busy = 0u;
        if (FW_MIC_SIDE() == 2u) for (unsigned i = 1; i < 7u; i++) busy |= AUDIO_APP_SLOTS[i];
        if (!busy) FW_CODEC_CTRL(0u);
        return;
    }
...
static void mic_send_status_notify(customCfwContext *ctx) {
     ...
     * [3] connected, [4..5] interval (1.25 ms units), [6..7] latency, [8..9] timeout (10 ms),
     * [10] mode (0xa3 fast / 0xa4 slow), [11] this temple's side. */
    {
        unsigned char *l = (unsigned char *)cfw_malloc(12u);
        if (l) {
...
void mic_apply_control(const uint8_t *data, uint32_t len) {
    ...
    if (op == MIC_OP_CONFIGURE) {
        ...
        if (ctx->mic_hw_armed && (ctx->mic_flags & MIC_FLAG_MGR_CTRL) && !*CODEC_I2S_INIT)
            FW_CODEC_CTRL(1u);
