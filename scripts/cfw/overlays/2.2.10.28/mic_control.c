...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        ctx->mic_hw_armed = 1;
        /* 2.2.10.28: bring the codec up the exact way the stock audio manager's acquire does —
         * codec-prep (GX8002 configuration) THEN ctrl(1) (I2S on + init). Every earlier CFW
         * candidate posted only ctrl(1) and skipped the prep, so the GX8002 was never configured
         * and the I2S DMA read its free-running full-scale noise (the array-capture root cause,
         * measured 2026-09-14). This is the CFW capture path and it fully replaces the stock mic:
         * the app never hands back to the stock 1-mic interface, so we keep the codec up and the
         * tap in slot 0 for the whole session. prep+ctrl have no side gate, so this is correct on
         * both temples (the LEFT-only service_audio_manager acquire cannot be reused on the RIGHT). */
        FW_CODEC_PREP(1u);
