...
#define MIC_FLAG_PUBLIC_MASK   (MIC_FLAG_BEAMFORM | MIC_FLAG_ARM_HW | MIC_FLAG_MGR_CTRL | MIC_FLAG_PHY_2M)
...
uint32_t mic_peer_sync_hook(const uint8_t *data, uint32_t len) {
    customCfwContext *ctx = peekCustomCfwContext();
    if (ctx && FW_MIC_SIDE() == 1u && (ctx->mic_flags & MIC_FLAG_ARM_HW) && ctx->mic_codec_cycled) {
...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (!ctx->mic_codec_cycled) {
            ...
            FW_CODEC_CTRL(2u);                      /* one-shot GX8002 power-cycle (cleans both temples) */
...
void mic_settle_tick(void *arg) {
    ...
    if (ctx->mic_settle_stage == 1u && FW_MIC_SIDE() == 1u) {   /* RIGHT: I2S off→on on the booted chip */
