...
#define MIC_LEFT_SETTLE_MS 12000u    /* 2.2.10.54: LEFT waits out the RIGHT's reboot + the right->left sync (as .40/.41) */
...
uint32_t mic_peer_sync_hook(const uint8_t *data, uint32_t len) {
    customCfwContext *ctx = peekCustomCfwContext();
    if (ctx && FW_MIC_SIDE() == 1u && (ctx->mic_flags & MIC_FLAG_ARM_HW) && (ctx->mic_flags & MIC_FLAG_RIGHT_DEAF) && ctx->mic_codec_cycled) {
        ctx->mic_peer_sync_ignored++;
        return 0u;
    }
...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (!ctx->mic_codec_cycled) {
            ...
            /* 2.2.10.56: BOTH temples run the stock reboot again. With the LEFT plain (.54/.55)
             * the RIGHT's DMA never started (codecOn=1 i2s=1 dma=0 for the whole run): the
             * RIGHT's capture is released by the LEFT owner's post-reboot peer handshake, which
             * only a LEFT codec reboot emits. Every earlier "both noisy" reading for this shape
             * was taken on the reboot-raced bench, so it is re-measured here on the fixed bench
             * with the mode-11 teardown gone. The LEFT re-acquires the manager after its reboot
             * (its cycle drops the codec/DMA the manager owns) and runs the same I2S
             * deinit/reinit settle as the RIGHT. */
            FW_CODEC_CTRL(2u);
            ...
            ctx->mic_codec_ready_tick = FW_MS_TICK + MIC_CODEC_SETTLE_MS
                + (FW_MIC_SIDE() == 2u ? MIC_LEFT_EXTRA_MS : 0u);
        }
        {
...
static void mic_complete_bringup(customCfwContext *ctx) {
    /* 2.2.10.56: LEFT re-acquires the audio manager (its reboot dropped the codec/DMA it owns). */
...
void mic_settle_tick(void *arg) {
    ...
    if (ctx->mic_settle_stage == 1u) {              /* 2.2.10.56 BOTH: I2S off->on on the rebooted chip */
        FW_CODEC_CTRL(0u);                          /* I2S deinit on the now-booted chip */
...
static void mic_session_stop(customCfwContext *ctx) {
    ...
    if (FW_MIC_SIDE() == 2u) FW_AUDM_RELEASE(MIC_AUDM_APP_ID);   /* 2.2.10.56: LEFT releases its manager slot */
