...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        /* 2.2.10.32: symmetric, self-owned codec bring-up on BOTH temples. 2.2.10.31 proved the
         * GX8002 codec-chip reboot (op 2) clears the wedged state that made capture full-scale
         * noise — the RIGHT temple went clean (RMS ~0.005 in a quiet room). But the LEFT, on its
         * separate slot-hold/stock-sharing path, never streamed, and the RIGHT streamed ~20 s
         * late because the stock 2 s codec-audio-check kept rebooting the codec until the tap
         * settled it. So both temples now use the identical self-owned path: reboot the GX8002,
         * reconfigure it (prep), enable I2S (ctrl 1), and immediately clear CODEC_ON_FLAG so the
         * codec-audio-check does not reboot-loop while the codec settles (the tap keeps it clear
         * once frames flow). No audio-manager slot hold and no dependence on the stock mic — the
         * four-mic array fully owns the codec on a CFW build, on both temples equally. */
