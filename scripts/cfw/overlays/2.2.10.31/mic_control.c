...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (FW_MIC_SIDE() == 2u) AUDIO_APP_SLOTS[AUDIO_APP_SLOT_CFW] = 1u;  /* LEFT: hold a manager slot */
        /* 2.2.10.31: reboot the GX8002 voice codec, then bring it up fully ourselves. The array
         * capture was intermittently full-scale noise across the day even on capture code that
         * had produced clean audio earlier — the DMA reading the GX8002's free-running output.
         * The GX8002 is a separate chip with its own firmware; op 2 is its full reboot (~1.75 s
         * blocking, the same path the stock 2 s codec-audio-check uses to recover a dead codec).
         * Nothing this session triggered it — the "reboot" the phone sends restarts the Apollo
         * host, not the GX8002 — so a wedged codec-chip state persisted. Reboot it, reconfigure it
         * (prep), then enable I2S (ctrl 1): a guaranteed cold, clean codec regardless of prior
         * state. Posted to the audio thread and processed in order. */
        FW_CODEC_CTRL(2u);                         /* GX8002 full reboot — clears a wedged codec */
        FW_CODEC_PREP(1u);                         /* reconfigure the GX8002 */
        ctx->mic_flags |= MIC_FLAG_WE_POWERED;
        FW_CODEC_CTRL(1u);                          /* I2S output on + init */
        return;
    }
...
static void mic_session_stop(customCfwContext *ctx) {
    ...
    /* 2.2.10.30: release the audio-manager app-slot we held on the LEFT so a later stock
     * acquire/release accounts correctly. On the RIGHT this cell is unused. */
    if (FW_MIC_SIDE() == 2u) AUDIO_APP_SLOTS[AUDIO_APP_SLOT_CFW] = 0u;
    if (ctx->mic_source == MIC_SRC_CODEC) {
