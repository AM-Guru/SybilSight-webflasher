...
void mic_settle_tick(void *arg) {
    ...
    /* 2.2.10.58: late I2S deinit/reinit on BOTH temples. The manager re-open (LEFT acquire, and
     * the peer re-open it sends the RIGHT) initialises I2S immediately, before the GX8002 has
     * finished its calibration; the full-scale-peak / 0.25-RMS "noise" is the I2S word
     * misalignment that leaves behind, and a later ctrl(1) is a no-op while I2S is already on.
     * So once the settle expires: I2S off, a short gap, then prep + I2S on + tap. */
    if (ctx->mic_settle_stage == 1u) {
        FW_CODEC_CTRL(0u);                          /* I2S deinit on the calibrated chip */
