...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (!ctx->mic_codec_cycled) {
            ...
            /* 2.2.10.48: NO rail power-cycle. The power-cycle was the only thing that ever dropped
             * a temple's phone link (a shared-rail / inter-temple disturbance), so test the
             * hypothesis that the codec never needed the rail reboot — only I2S initialised AFTER
             * the GX8002 finished its post-configure MicDelay calibration. Bring the codec up the
             * clean stock (acquire) way — prep (GX8002 config, starts calibration) + ctrl(1) — then
             * let the settle deinit/reinit I2S on the calibrated chip before tapping. If this is
             * clean, there is no rail cycle and therefore no link drop on either temple. */
            { unsigned char mk[4] = { 'M','K', (unsigned char)MIC_PROTO_VERSION, (unsigned char)FW_MIC_SIDE() };
              FW_AUDIO_NOTIFY(mk, 4u); }
            if (FW_MIC_SIDE() == 2u) FW_AUDM_ACQUIRE(MIC_AUDM_APP_ID);  /* LEFT owner: coordinated acquire */
            FW_CODEC_PREP(1u);                      /* configure the GX8002 (starts calibration) */
            ctx->mic_flags |= MIC_FLAG_WE_POWERED;
            FW_CODEC_CTRL(1u);                      /* I2S on (early; the settle re-inits it clean) */
...
void mic_settle_tick(void *arg) {
    ...
    if (ctx->mic_settle_stage == 1u) {             /* 2.2.10.48 BOTH: I2S off→on on the calibrated chip */
        FW_CODEC_CTRL(0u);                          /* I2S deinit */
