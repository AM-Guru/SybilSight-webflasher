...
#define MIC_UNREG_GRACE_MS     200u
...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        ctx->mic_hw_armed = 1;
        /* Open the codec only if it is not already running (I2S init flag clear). */
        if (!*CODEC_I2S_INIT) {
...
static void mic_session_stop(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
         ...
         * itself unregisters on its next call, the watchdog timer covers the no-frames case. */
        ...
        if (ctx->mic_watchdog_timer) {
            ...
            FW_TIMER_START(ctx->mic_watchdog_timer, MIC_UNREG_GRACE_MS);
        } else {
            mic_finish_unregister(ctx);
        }
        return;
    }
    FW_PCM_UNREGISTER(MIC_STOCK_OWNER_ID, MIC_PCM_SLOT);
