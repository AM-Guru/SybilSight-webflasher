...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        ctx->mic_hw_armed = 1;
        /* 2.2.10.27: a stock mic release on the LEFT leaves the codec HALF-powered — the chip
         * still powered (CODEC_POWER_ON set) but the I2S torn down (CODEC_I2S_INIT clear). From
         * that state the codec-control on-path takes its "already powered" branch and the I2S
         * DMA streams the GX8002's full-scale noise for the whole session (measured 2026-09-14:
         * every armed session reached the recognizer at peak 1.0). Stock never hits this because
         * production init always arms from a cold codec. So force a cold codec here: if it is
         * powered at all, deinit fully first (ctrl(0) = I2S deinit + power down, a cheap no-op on
         * an already-off codec because both its steps guard on their own state flag), then do the
         * clean cold init (ctrl(1)). We own the power either way, so mark WE_POWERED. */
        if (*CODEC_POWER_ON) FW_CODEC_CTRL(0u);
...
void mic_apply_control(const uint8_t *data, uint32_t len) {
    ...
    if (op == MIC_OP_CONFIGURE) {
        ...
        if (ctx->mic_hw_armed && ctx->mic_source == MIC_SRC_CODEC && !*CODEC_I2S_INIT) {
            /* 2.2.10.27: a stock release under the armed tap can leave the codec half-powered
             * (power on, I2S down); cold-cycle it the same way session_start does. */
            if (*CODEC_POWER_ON) FW_CODEC_CTRL(0u);
            ctx->mic_flags |= MIC_FLAG_WE_POWERED;
            FW_CODEC_CTRL(1u);
        }
