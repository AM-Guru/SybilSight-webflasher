...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        ctx->mic_hw_armed = 1;
        /* 2.2.10.29: if the codec is ALREADY UP (I2S initialised), the stock LC3 path has it
         * running cleanly and its DMA is producing valid stereo samples. Just take PCM slot 0
         * (done above) and leave the codec completely alone — do NOT prep/ctrl it. Registering
         * the tap replaced the stock LC3 encoder callback, so no stock frames are produced, but
         * the codec and its I2S stay up and clean and the tap gets the valid samples. This is the
         * CFW capture model: the four-mic array replaces the stock mic by stealing its slot on a
         * live codec, never by tearing the codec down and re-initialising it (every re-init left
         * I2S uninitialised and the DMA streamed full-scale noise, measured 2026-09-14 .18–.28).
         * The app keeps the stock codec enabled for the whole session so this branch is taken. */
        if (*CODEC_I2S_INIT) {
            ctx->mic_flags &= (uint8_t)~MIC_FLAG_WE_POWERED;  /* stock owns the power; we only tap */
            return;
        }
        /* Codec was down (no stock path up): bring it up the exact stock way — codec-prep
         * (GX8002 configuration) THEN ctrl(1) (I2S on + init). */
        FW_CODEC_PREP(1u);
