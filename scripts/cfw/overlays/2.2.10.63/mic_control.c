...
static void relay_tap(customCfwContext *ctx, const void *pcm, uint32_t bytes) {
    ...
    if ((++ctx->relay_stat_ctr % 40u) == 0u) {                    /* 2.2.10.63: BOTH sides emit RS */
        ...
        st[20] = (uint8_t)ctx->relay_bad_len; st[21] = (uint8_t)(ctx->relay_bad_len >> 8);
        st[22] = (uint8_t)ctx->relay_last_offset_ms; st[23] = (uint8_t)((uint16_t)ctx->relay_last_offset_ms >> 8);
        FW_AUDIO_NOTIFY(st, 24u);
    }
...
uint32_t mic_peer_sync_hook(const uint8_t *data, uint32_t len) {
    { customCfwContext *hc = peekCustomCfwContext(); if (hc) hc->relay_hook_calls++; }   /* 2.2.10.63 */
