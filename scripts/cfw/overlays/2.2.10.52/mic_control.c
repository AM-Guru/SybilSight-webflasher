...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (!ctx->mic_codec_cycled) {
            ...
            ctx->mic_codec_ready_tick = FW_MS_TICK + MIC_PWR_OFF_MS + MIC_CODEC_SETTLE_MS
            ...
            FW_CODEC_PWR_ON();                       /* no timer: degrade to an immediate power-on */
        }
        {
...
static void mic_complete_bringup(customCfwContext *ctx) {
    /* 2.2.10.46: LEFT re-acquires the audio manager (its cycle disabled the codec/DMA it owns). */
...
void mic_settle_tick(void *arg) {
    ...
    if (ctx->mic_settle_stage == 3u) {              /* 2.2.10.52: off-dwell elapsed -> power on */
        ...
        ctx->mic_settle_stage = 1u;                 /* now waiting for the chip to boot + calibrate */
        {
            ...
            if (ctx->mic_settle_timer) FW_TIMER_START(ctx->mic_settle_timer, wait ? wait : 1u);
        }
        return;
    }
    if (ctx->mic_settle_stage == 1u) {              /* 2.2.10.52 BOTH: I2S off->on on the booted chip */
        FW_CODEC_CTRL(0u);                          /* I2S deinit on the now-booted chip */
        ctx->mic_settle_stage = 2u;
        if (ctx->mic_settle_timer) FW_TIMER_START(ctx->mic_settle_timer, MIC_I2S_REINIT_GAP_MS);
        return;
    }
...
static void mic_session_stop(customCfwContext *ctx) {
    ...
    if (FW_MIC_SIDE() == 2u) FW_AUDM_RELEASE(MIC_AUDM_APP_ID);   /* 2.2.10.46: LEFT release its manager slot */
