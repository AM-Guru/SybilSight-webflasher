...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        /* 2.2.10.34: plain bring-up on BOTH temples, no GX8002 reboot. 2.2.10.31/.33 showed the
         * codec reboot un-wedges a codec stuck in full-scale noise, but it also destabilises the
         * rebooting temple's BLE link (~1.75 s blocking → the pair drops/reboots) and fires the
         * inter-temple DMIC-close. Once a reboot has un-wedged the GX8002 the chip stays clean
         * across Apollo-host soft reboots (its state is separate), so steady-state arming needs
         * only a plain configure + I2S enable — no reboot, no link disruption. A wedged codec is
         * recovered out of band (the phone detects the full-scale-noise signature and asks for a
         * targeted GX8002 reboot) rather than on every arm. */
