...
#define FW_AUDM_ACQUIRE      ((audm_fn)0x005681A9U)   /* rebased to 0x0056AC65 */
...
#define FW_CODEC_PWR_OFF     ((codec_pwr_step_fn)0x00592DB5U)    /* gx8002 power off; rebased to 0x005959B9 */
#define FW_CODEC_PWR_ON      ((codec_pwr_step_fn)0x00592D01U)    /* gx8002 power on;  rebased to 0x00595905 */
#define FW_AUDM_RELEASE      ((audm_fn)0x00568337U)   /* rebased to 0x0056ADF3 */
...
#define FW_DM_SET_PHY        ((dm_set_phy_fn)0x004DB279U)
...
typedef uint32_t (*common_data_fn)(const uint8_t *data, uint32_t len);
#define FW_AUDM_PEER_MSG_ORIG ((common_data_fn)0x0056873BU)
/* 2.2.10.62: inter-temple common-data send, as used by the audio manager's peer-sync sender
 * (2.2.10.10 0x46b0ac, 2.2.9.22 0x46aac8, identical bodies): send(record_id, buf, len, 0). */
typedef int (*peer_send_fn)(uint32_t record_id, const void *buf, uint32_t len, uint32_t opt);
#define FW_PEER_SEND         ((peer_send_fn)0x0046AAC9U)
/* 2.2.10.62: stock LC3 wrapper SVC_Lc3EncodeMono (2.2.10.10 0x595d3c, 2.2.9.22 0x593138):
...
#define FW_LC3_ENCODE_MONO   ((lc3_encode_fn)0x00593139U)
...
#define MIC_FLAG_PUBLIC_MASK   (MIC_FLAG_BEAMFORM | MIC_FLAG_ARM_HW | MIC_FLAG_MGR_CTRL | MIC_FLAG_PHY_2M | MIC_FLAG_RIGHT_DEAF)
...
#define RELAY_PAIR_WINDOW    60      /* ms: max |right chunk time - left chunk time| to pair */
...
static uint8_t relay_fbytes_for(const customCfwContext *ctx) {
    ...
    if (fb < 20u) fb = 20u;
    if (fb > RELAY_MAX_FBYTES) fb = RELAY_MAX_FBYTES;
    return (uint8_t)fb;
}
...
static void relay_send_ctl(uint8_t magic, uint32_t tick, uint8_t fb) {
    ...
    FW_PEER_SEND(RELAY_RECORD_ID, b, 6u, 0u);
}
...
/* LEFT: take the RIGHT chunk nearest in time to `tick` within the pairing window. */
static uint8_t *relay_ring_take(customCfwContext *ctx, uint32_t tick, int *offset_ms) {
    uint8_t *best = 0; int best_d = RELAY_PAIR_WINDOW + 1;
    for (unsigned i = 0; i < RELAY_RING_N; i++) {
        uint8_t *en = ctx->relay_ring + i * RELAY_ENTRY_BYTES;
        if (!en[0]) continue;
        int d = (int)(relay_rd32(en + 4) - tick);
        int ad = d < 0 ? -d : d;
        if (ad < best_d) { best_d = ad; best = en; *offset_ms = d; }
        else if (d < -RELAY_PAIR_WINDOW) en[0] = 0u;   /* too old: drop */
    }
    return best;
}
...
static void relay_tap(customCfwContext *ctx, const void *pcm, uint32_t bytes) {
    ...
    /* LEFT: re-issue START until the RIGHT acknowledges */
    if (!ctx->relay_started && (now - ctx->relay_start_tick) > RELAY_START_RETRY_MS) {
    ...
    if (off > 127) off = 127;
    if (off < -127) off = -127;
    pay[0] = (uint8_t)nf; pay[1] = fb; pay[2] = (uint8_t)(int8_t)off; pay[3] = present;
    uint32_t paylen = 4u + 4u * nf * fb;
    uint16_t seq = (uint16_t)ctx->mic_frames;
    o[0] = MIC_STREAM_MAGIC0; o[1] = MIC_STREAM_MAGIC1; o[2] = MIC_PROTO_VERSION;
    o[3] = (uint8_t)(MIC_FLAG_ARM_HW | MIC_STREAM_LEFT | MIC_STREAM_RELAY);
    o[4] = (uint8_t)seq; o[5] = (uint8_t)(seq >> 8);
    relay_wr32(o + 6, now);
    o[10] = 4u;                                                  /* channels */
    o[11] = (uint8_t)MIC_RATE_DEFAULT; o[12] = (uint8_t)(MIC_RATE_DEFAULT >> 8);
    o[13] = 0u;                                                  /* S16 */
    o[14] = MIC_CODEC_LC3;
    o[15] = 0xFFu; o[16] = 0x7Fu;                                /* angle: not computed here */
    o[17] = (uint8_t)off; o[18] = (uint8_t)((uint16_t)(int16_t)off >> 8);   /* ssr field carries the offset */
    o[19] = (uint8_t)paylen; o[20] = (uint8_t)(paylen >> 8);
    FW_AUDIO_NOTIFY(o, MIC_STREAM_HDR_BYTES + paylen);
    ctx->mic_frames++;
    ctx->mic_last_tap_tick = now;
    if ((ctx->mic_frames % 40u) == 0u) {                          /* every ~2 s: relay stats */
        uint8_t st[20];
        ...
        st[6] = (uint8_t)ctx->relay_paired; st[7] = (uint8_t)(ctx->relay_paired >> 8);
        st[8] = (uint8_t)ctx->relay_missing; st[9] = (uint8_t)(ctx->relay_missing >> 8);
        st[10] = (uint8_t)ctx->relay_enc_fail; st[11] = (uint8_t)(ctx->relay_enc_fail >> 8);
        st[12] = (uint8_t)ctx->relay_bad_len; st[13] = (uint8_t)(ctx->relay_bad_len >> 8);
        st[14] = (uint8_t)ctx->relay_last_offset_ms; st[15] = (uint8_t)((uint16_t)ctx->relay_last_offset_ms >> 8);
        st[16] = ctx->relay_started; st[17] = fb; st[18] = (uint8_t)nf; st[19] = 0u;
        FW_AUDIO_NOTIFY(st, 20u);
    }
}
...
static int relay_handle_peer(customCfwContext *ctx, const uint8_t *data, uint32_t len) {
    ...
    if (magic == RELAY_MAGIC_START && len >= 6u) {
        if (FW_MIC_SIDE() == 1u) {
            ...
            ctx->relay_offset_ticks = (int32_t)(left_tick - now);   /* + link transit (~1 ms) */
            ctx->relay_seq = 0u; ctx->relay_started = 1u;
            relay_send_ctl(RELAY_MAGIC_ACK, now, fb);
        }
        return 1;
    }
...
uint32_t mic_peer_sync_hook(const uint8_t *data, uint32_t len) {
    ...
    return FW_AUDM_PEER_MSG_ORIG(data, len);
}
