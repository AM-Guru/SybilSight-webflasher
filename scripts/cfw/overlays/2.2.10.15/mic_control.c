...
static uint32_t mic_pack_adpcm(const int16_t *pcm, uint32_t bytes, uint8_t nch, uint8_t decim, uint8_t *out, uint32_t cap) {
    uint32_t total = bytes / 2u;                 /* int16 samples across channels */
    uint32_t per_ch = nch == 2u ? total / 2u : total;
    uint32_t out_samples = decim == 2u ? per_ch / 2u : per_ch;
...
__attribute__((used, noinline)) void mic_pcm_tap(uint32_t source, const void *pcm, uint32_t bytes) {
    ...
    if (!f) return;
    uint32_t pay = mic_pack_adpcm((const int16_t *)pcm, bytes, nch, decim, f + MIC_STREAM_HDR_BYTES, 6u + MIC_ADPCM_MAX_SAMPLES);
    if (bytes / 2u / (nch == 2u ? 2u : 1u) / decim > MIC_ADPCM_MAX_SAMPLES) flags |= MIC_STREAM_TRUNC;
