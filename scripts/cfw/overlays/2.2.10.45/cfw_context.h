...
typedef struct {
    ...
    uint32_t mic_layout_rev;                /* 2.2.10.45: bump to force a fresh context after a
                                             * re-flash whose struct size happens to match the old
                                             * one (ctx_size alone can collide across candidates) */
} customCfwContext;
