...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (FW_MIC_SIDE() == 2u) {                 /* LEFT: register as an audio-manager client */
            AUDIO_APP_SLOTS[AUDIO_APP_SLOT_CFW] = 1u;
            if (*CODEC_I2S_INIT) {                 /* stock already brought the codec up cleanly */
                ctx->mic_flags &= (uint8_t)~MIC_FLAG_WE_POWERED;
                return;
            }
        }
        /* Codec is down (RIGHT temple, or LEFT with no stock path up): bring it up the exact
         * stock way — codec-prep (GX8002 configuration) THEN ctrl(1) (I2S on + init). */
        FW_CODEC_PREP(1u);
