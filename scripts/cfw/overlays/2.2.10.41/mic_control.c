...
#define FW_CODEC_PWR_OFF     ((codec_pwr_fn)0x00592DB5U)    /* gx8002 power off; rebased to 0x005959B9 */
#define FW_CODEC_PWR_ON      ((codec_pwr_fn)0x00592D01U)    /* gx8002 power on;  rebased to 0x00595905 */
...
#define MIC_PWR_OFF_MS 100u          /* 2.2.10.41: power-off dwell before power-on (as the stock reboot handler) */
...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (!ctx->mic_codec_cycled) {
            ...
            if (FW_MIC_SIDE() == 1u) FW_CODEC_PWR_OFF();  /* RIGHT: non-blocking power-cycle, step 1 */
            ...
            if (FW_MIC_SIDE() == 1u) {
                ...
                FW_CODEC_PWR_ON();                           /* no timer: degrade to a blocking cycle */
            }
        }
        {
...
void mic_settle_tick(void *arg) {
    ...
    if (ctx->mic_settle_stage == 3u) {              /* 2.2.10.41 RIGHT: off-dwell elapsed → power on */
        FW_CODEC_PWR_ON();
