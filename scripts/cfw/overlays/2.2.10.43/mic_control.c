...
static void mic_session_start(customCfwContext *ctx) {
    ...
    if (ctx->mic_source == MIC_SRC_CODEC) {
        ...
        if (!ctx->mic_codec_cycled) {
            ...
            /* 2.2.10.43: BOTH temples reset their own codec (rail-preserving). A right-only reset
             * dropped the right's phone link every time (.40–.42) while resets on both temples at
             * once never did (.38/.39): the codec reset fires the right→left inter-temple audio
             * sync (close / low-power / reopen / init handshake, per openCFW); with the LEFT
             * healthy that handshake runs its full course and ties the RIGHT up past the 720 ms
             * supervision, with the LEFT also mid-reset it aborts at once. The LEFT's own reset
             * only silenced it before because it dropped the shared rail (pin 6); pins 7/8 keep
             * the rail up, so the LEFT can now reset safely too. The phone arms right-first, then
             * left ~1.5 s later, so both are mid-reset together. */
            ...
            ctx->mic_codec_ready_tick = FW_MS_TICK + MIC_PWR_OFF_MS + MIC_PIN_STAGGER_MS + MIC_CODEC_SETTLE_MS;
            {
